...
```{r error=TRUE, eval=FALSE, message=TRUE}
outer_function(3)
```
...
For example, I specify the type of an object at the end of an object name, following `_`:
```{r eval = FALSE}
# regression summary, denoted by "_lm"
heightMass_lm <- starwars %>% 
  lm(height ~ mass,
     data = .) %>% 
  summary

# dataset, denoted by "_dataset"
...
## Error 1 {.tabset .tabset-fade .tabset-pills}  

### [Object not found](http://varianceexplained.org/courses/errors/)
...
## Error 2 {.tabset .tabset-fade .tabset-pills}  
### [Function not found](http://varianceexplained.org/courses/errors/)  
```{r echo=TRUE, eval=FALSE, message=TRUE, error=TRUE}
some_function(x)
```
...
```
```{r echo=FALSE}
# TODO: find common problem functions  
```

## Error 3 {.tabset .tabset-fade .tabset-pills}  
### Syntax errors  

```{r eval = FALSE, message=TRUE, error=TRUE}
...
```{r, echo = TRUE, eval = TRUE, code = readLines("ts_error3.R")}
```

Make sure each logical operator is accompanied by its corresponding object(s), and check your operators are correctly specified. For example, one equal sign (`r =`) is designated for object assignment; two equal signs (`r ==`) should be used to denote equal to. 

## Error 4 {.tabset .tabset-fade .tabset-pills}  
### Prompt sign change from `>` to `+` 
...
```{r, echo = TRUE, eval = TRUE, code = readLines("ts_error4a.R")}
```

b) check your quotes   
```{r, echo = TRUE, eval = TRUE, code = readLines("ts_error4b.R")}
```

c) Or hit `esc` key  
This will cancel the incomplete code, and allows you to inspect the code.  

## Error 5 {.tabset .tabset-fade .tabset-pills}  
### `cannot open`  
...
## Error 6 {.tabset .tabset-fade .tabset-pills}    
...
### Solution  
This error usually denotes nonexistent element; check you are trying to access existing elements.  
```{r}
dim(starwars)
```
The first corresponds to the number of rows (observations) and the second corresponds to the number of columns (variables).  

## Error 7 {.tabset .tabset-fade .tabset-pills}   
### Missing values    
...
### Solutions  
a) 
```{r, echo = TRUE, eval = TRUE, code = readLines("ts_error7a.R")}
```

b) 
```{r, echo = TRUE, eval = TRUE, code = readLines("ts_error7b.R")}
```
